...
# Log file
sink(paste0('MiADE_generate_additional_synonyms', Sys.time(), '.log'),
	split = TRUE)

# Import Freetext Matching Algorithm synonyms
downloadFMA()
# Synonyms with at least two words (single word synonyms are best
# handled by MedCat disambiguation
#SYNONYMS <- fread(paste0(tempdir(), '/fma/synonyms.txt'))[priority >= 3,
#	.(text, sct = read)][(text %like% ' ') | (sct %like% ' ')]
...
SNOMED <- loadSNOMED(c(
  'Feb2022/SnomedCT_UKClinicalRF2_PRODUCTION_20220216T000001Z/',
  'Feb2022/SnomedCT_InternationalRF2_PRODUCTION_20210131T120000Z'
  )
  , active_only = TRUE)
...
# List of suspected, negated, historic and family history concepts to
...
add_alternate_word_order <- function(SCT){
	...
	SCT[, words := strsplit(tolower(term), ' ')]
	# Strip 'the', 'and', 'of'
...
#### PROCESS SNOMED CONCEPTS - ADD SYONYMS ####
...
# Not using FMA synonyms - but can use manual synonyms if desired
# SCT <- add_synonyms(SCT, SYNONYMS$text, SYNONYMS$sct)
...
names(SCT)[names(SCT) == "conceptId"] <- "cui"
names(SCT)[names(SCT) == "term"] <- "name"
...
OUT <- merge(SCT, rbind(SNOMED$DESCRIPTION[, .(conceptId, term = term,
